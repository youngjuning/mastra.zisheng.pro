...
| `max_tokens`        | integer                 | ❌ 否 | 模型最大上下文 | 限制**生成**的最大 token 数（不含输入）                   |
...
| `stop`              | string \| Array<string> | ❌ 否 | `null`         | 遇到这些字符串立即停止生成                                |
